...
description: Build a clean, reviewed, segmented professional contact list from Outlook, Microsoft 365, Gmail, Google Workspace, Apple Mail, or exported MBOX email history for Sunshine Island CRM. Use when an architect wants Codex to find relevant contacts and interactions, suggest AMI relationship tags, apply an optional user-approved set of project-type or client-type tags, match likely duplicate identities, collect and format phone numbers, create a workbook and CSV, or prepare for a GoHighLevel contact import.
---
...
Use batches small enough for reliable progress and save after each completed batch. If a connector paginates or limits results, continue through every page in the approved scope. Keep a brief progress log containing the date range completed, folders searched, and the last stable output version.
...
After approval, process the remaining approved date range in batches. Preserve stable identifiers and previously approved decisions. Recheck new evidence that may strengthen or change a contact's category.
